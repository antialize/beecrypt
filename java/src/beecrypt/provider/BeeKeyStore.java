...
public class BeeKeyStore extends KeyStoreSpi
{
	...
	public void engineSetKeyEntry(String alias, Key key, char[] password, Certificate[] chain) throws KeyStoreException
	{
		if (key instanceof PrivateKey)
		{
			...
			throw new KeyStoreException("BeeKeyStore only supports storing of PrivateKey objects");
	}
	...
	public void engineLoad(InputStream in, char[] password) throws IOException, CertificateException, NoSuchAlgorithmException
	{
		if (in == null)
		{
		...
		else
		{
				...
				case BKS_PRIVATEKEY_ENTRY:
					{
						...
						if (keySize <= 0)
							throw new IOException("Invalid BeeKeyStore encoded key size");
						...
						for (int j = 0; i < certCount; j++)
						{
							...
							if (certSize <= 0)
								throw new IOException("Invalid BeeKeyStore encoded certificate size");
			...
			if (macSize != computedMac.length)
				throw new IOException("BeeKeyStore has been tampered with, or password was incorrect (incorrect mac size)");
	...
	public void engineStore(OutputStream out, char[] password) throws IOException, CertificateException, NoSuchAlgorithmException
	{
		...
		while (it.hasNext())
		{
			...
			throw new ProviderException("entry is neither KeyEntry nor CertEntry");
		}
